...
```jsonc
{
  ...
  "rawWireLog": true           // 와이어 원문 기록 — 아래 "와이어 원문" 참고
}
```
...
를 ③ 칸만으로는 가릴 수 없습니다. ④ 칸이 그 자리입니다. `rawWireLog`(기본 켜짐)가 켜져
있으면 한 호출당 두 쪽이 **가공 없이** 남습니다.

| 쪽 | 무엇 | 스트리밍이면 |
| --- | --- | --- |
| 사내 → 프록시 | 상태 줄 · 헤더 · 본문 바이트 그대로 | `data:` 줄까지 원문 그대로 |
| 프록시 → 클라이언트 | 클라이언트가 실제로 읽은 본문 | 우리가 쓴 `data:` 줄 그대로 (`[DONE]` 포함) |
...
**메모리에만** 있고 앱을 끄면 사라집니다. 메모리를 아껴야 하면 설정에서 끕니다(끄면 ④ 칸이
"기록이 꺼져 있습니다"로 바뀝니다 — 켰는데 안 온 것과 헷갈리지 않습니다).
...
| 로그 ③ 칸의 `원본 JSON 보기` | ③ 은 `전체보기`, 원본은 **④ 칸** | ③ 칸이 들고 있는 것은 가공된 답변 전문이라 "원본"이라 부르면 거짓말입니다. 그래서 ③ 은 가진 그대로 "전체보기"로 부르고, 사내가 준 JSON 봉투와 클라이언트로 나간 본문은 ④ 칸에 **따로** 둡니다 (위 「와이어 원문」). |
...
> 실서버 응답을 뜨는 가장 빠른 길은 로그 창 **④ 와이어 원문**입니다(기본 켜짐). 호출 한 건을
